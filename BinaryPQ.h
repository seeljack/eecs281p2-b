...
private:
    ...
    void fixUp(std::vector<TYPE> data, size_t index){
        while(index > 0 && this->compare(data[getElement(index /2)],data[getElement(index)])){
            swap(data[getElement(index)], data[getElement(index /2)]);
            index /= 2;
        }
    }

    void fixdDown(std::vector<TYPE> data, int size, int index){
        while(2*index <= size){
            int j = 2 * index;
            if(j < size && this->compare(data[getElement(index)],data[getElement(index + 1)])){
                j++;
            }
            if(this->compare(data[getElement(j)],data[getElement(index)]) || data[getElement(j)] == data[getElement(index)]){
                break;
            }
            swap(data[getElement(index)],data[getElement(j)]);
            index = j;
        }
    }
}; // BinaryPQ
